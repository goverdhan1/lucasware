...
module.exports = {
    ...
    headerCell: function( gridId, expectedCol, expectedValue ) {
        return element( by.id( gridId ) ).element( by.css('.ui-grid-render-container-body')).element( by.css('.ui-grid-header') ).element( by.repeater('col in colContainer.renderedColumns track by col.colDef.name').row( expectedCol)).element(by.css('.ui-grid-cell-contents'));
    },
    ...
    /**
     * LUCAS custom selectors
     */

    /**
     * Selects/un-selects a grid row. Requires 'selection' module to be included
     * otherwise this will have no affect
     */
    clickGridRow: function(gridId, rowNumber) {
        var row = element(by.id(gridId))
            .element(by.css('.ui-grid-render-container-body'))
            .element(by.css('.ui-grid-viewport'))
            .element(by.repeater('(rowRenderIndex, row) in rowContainer.renderedRows track by $index').row(rowNumber))
            .element(by.css('.ui-grid-cell'));
  ...
  /**
     ...
     * otherwise this will have no affect
     */
    clickGridRowHeader: function(gridId, rowNumber) {
        var row = element(by.id(gridId))
            .element(by.css('.ui-grid-pinned-container'))
            .element(by.css('.ui-grid-viewport'))
            .element(by.repeater('(rowRenderIndex, row) in rowContainer.renderedRows track by $index').row(rowNumber))
            .element(by.css('.ui-grid-selection-row-header-buttons'));
     ...
     *
     */
    expectRowCount: function(gridId, expectedNumberOfRows) {
        var rows = element(by.id(gridId))
            .element(by.css('.ui-grid-render-container-body'))
            .element(by.css('.ui-grid-viewport'))
            .all(by.repeater('(rowRenderIndex, row) in rowContainer.renderedRows track by $index'));
        expect(rows.count()).toEqual(expectedNumberOfRows);
    }
};
